...
const movoff = document.getElementById('movoff');
...
export function distReward(ai, target) {
...
export function reward(ai, target) {
...
function repetitionPunishment(ai) {
...
export function chooseAction(ai, epsilon = 0.1) {
    if (Math.random() < epsilon) {
        ...
        return (sortedEntries[0]?.[0] ?? 'up');
    }
}
function downloadQTable(ai) {
...
let testTarget;
let obstacles = [];
function genObstacles(count, color = 'red') {
    obstacles = [];
...
function loop() {
    ...
    switch (action) {
        case 'up':
            if (posPun = checkPos(testBot.x, testBot.y + 1))
                break;
            testBot.y += 1;
            break;
        case 'down':
            ...
            break;
        case 'left':
            if (posPun = checkPos(testBot.x - 1, testBot.y))
                break;
            testBot.x -= 1;
            break;
        case 'right':
...
file.addEventListener('change', (e) => {
    ...
    reader.onload = (event) => {
        setup();
        if (!event.target || !testBot)
            return;
        const content = event.target.result;
        const qTable = JSON.parse(content);
...
const ufrom = document.getElementById('UForm');
